...
本记录只证明固定代码提交 `a69858e33a5a027331e7c55b274d96e00142b93f` 在下列专用 Vercel / Supabase 资源上的一次公网验收。结果为 **GO**。没有购买、升级或修改任何无关项目；美国侧独立网络延迟没有观测条件，明确记为 **SKIP**。2026-09-03 后续凭据轮换已通过 Supabase Management API 与 Vercel Production redeploy 完成，原数据库密码轮换 **HOLD** 已关闭。
...
| Participant Vercel | Project `prj_7d3CY9Ufac7mElPp8zFcj9XdrABq` / Deployment `dpl_2PN54Lc91sVmdTKTTSt39JHPbJKF` | root `apps/participant-web` / Node 24 / Functions `sfo1` | `READY` |
| Admin Vercel | Project `prj_uTBUH1q87MwrtdOpdiab7Wf7Jo4T` / Deployment `dpl_6FX2VsXRuwUFQfzKdqp4qSpoAHV9` | root `apps/admin-web` / Node 24 / Functions `sfo1` | `READY` |

- Participant: <https://egocapture-participant.vercel.app>
- Admin: <https://egocapture-admin.vercel.app>
- 两个最终 deployment 均从无 tracked diff 的固定提交构建，并在凭据轮换后以 Production target 重新部署。
- 两套 Project 各有 19 个 Production 环境变量。按交付方明确要求，全部从 Sensitive 重建为可读取的 Config；普通变量仍由 Vercel 加密存储，但项目成员可在 Dashboard 中查看值。
- 本地保留 `.env.production.admin.local` 与 `.env.production.participant.local` 两份可直接加载的生产配置；两者权限均为 `0600`、受 `.gitignore` 排除，只在 `AUTH_COOKIE_NAME` 上保持应用隔离，并剔除了 Vercel/Turbo/Nx 系统变量。
...
- 后续轮换通过本机 Supabase CLI 的 Keychain 凭据调用官方 Management API，成功更新专用项目数据库密码；新 transaction-pooler 连接验证通过后，两个 Vercel Project 的 `DATABASE_URL` 同步更新并重新部署。过程中创建的临时本机 Keychain 条目已删除。
- Admin demo password 已按交付方指定值同步到 Supabase Auth 和两个 Vercel Project，但本记录不保存其明文；公网 Admin 登录验证通过。
- 一次本地 env 解析失败曾把旧 Marker private JWK 带入工具输出，随即重新生成整套 Marker key pair、更新 key id、同步双端 Production 并再次部署；最终运行时与本地生产 env 均只保留新密钥。
...
- **RESOLVED**：数据库密码、Admin demo password 与 Marker key pair 均已轮换；两个 Vercel Project 的 19 个 Production 变量均为 Config，最终 deployment 为 `READY`。
