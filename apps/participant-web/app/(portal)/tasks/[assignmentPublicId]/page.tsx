  ...
  return (
    <main className="content-page max-w-3xl">
      ...
      <section className="py-8">
        ...
        {openSessions.length > 0 ? <div className="mt-8"><h2 className="display text-2xl font-semibold">{i18n.t("participantUi.showQrCode")}</h2><div className="mt-4 grid gap-5 sm:grid-cols-2">{openSessions.map((session, index) => <Card as="article" key={session.publicId} className="gap-4 p-4"><div><Link href={`/sessions/${session.publicId}`} className="font-bold text-[var(--teal)]">{session.publicId} · {i18n.state("recording_session.status", session.status)}</Link><p className="mt-2 text-xs text-[var(--muted)]">{session.deviceLabel} · {i18n.t("participantUi.sessionMarker")} {session.markerAcknowledgedAt ? i18n.t("participantUi.markerAcknowledged") : i18n.t("participantUi.markerPending")}</p></div><div className="rounded-xl bg-white p-3"><Image src={markers[index].qrDataUrl} alt={i18n.t("participantUi.markerQrAlt", { session: session.publicId })} width={900} height={900} unoptimized className="mx-auto aspect-square w-full" /></div><Link href={{ pathname: "/uploads", query: { session: session.publicId } }} className={buttonVariants({ className: "w-full" })}>{i18n.t("participantUi.uploadVideo")}</Link></Card>)}</div></div> : null}
      </section>
    </main>
  );
}
