...
export default async function ParticipantTasksPage() {
  ...
  return (
    <main className="content-page max-w-3xl">
      ...
      <Link href="/uploads" className={buttonVariants({ className: " mt-6" })}>上传录制文件 →</Link>
      <div className="mt-10 space-y-4">
        {assignments.map((assignment) => (
          <Link key={assignment.publicId} href={`/tasks/${assignment.publicId}`} className="rounded-xl border bg-card text-card-foreground shadow-sm block p-6 transition hover:-translate-y-1 hover:shadow-[var(--shadow)]">
            <div className="flex justify-between gap-4"><p className="text-xs font-bold text-[var(--signal)]">{assignment.publicId}</p><Badge>{assignment.status}</Badge></div>
        ...
        {assignments.length === 0 ? <Empty><EmptyDescription>目前没有分配给你的任务。</EmptyDescription></Empty> : null}
      </div>
    </main>
  );
}
