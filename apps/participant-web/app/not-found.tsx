  ...
  return (
    <main className="flex min-h-[100dvh] items-center justify-center px-5 py-12">
      <Card className="w-full max-w-lg p-8 text-center sm:p-12">
        ...
        <h1 className="display mt-2 text-4xl font-semibold">{t("shell.participantNotFoundTitle")}</h1>
        <p className="mt-4 text-sm text-[var(--muted)]">{t("shell.participantNotFoundBody")}</p>
        <Link href="/" className={buttonVariants({ className: " mt-7" })}>{t("shell.backHome")}</Link>
      </Card>
    </main>
  );
}
