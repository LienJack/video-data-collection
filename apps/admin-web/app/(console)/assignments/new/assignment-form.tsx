...
export function AssignmentForm({ participants, versions, devices }: {
  ...
  return <form onSubmit={submit} className="mt-8 grid gap-6 border border-[var(--line)] bg-white/30 p-7 sm:grid-cols-2"><label className="text-sm font-bold">Participant<select name="participantPublicId" required className={field}>{participants.map((participant) => <option key={participant.publicId} value={participant.publicId}>{participant.displayAlias} · {participant.publicId}</option>)}</select></label><label className="text-sm font-bold">Published TaskVersion<select name="taskVersion" required className={field}>{versions.map((version) => <option key={`${version.taskPublicId}:${version.version}`} value={`${version.taskPublicId}:${version.version}`}>{version.taskTitle} v{version.version}</option>)}</select></label><label className="text-sm font-bold">Due At<input name="dueAt" type="datetime-local" required className={field} /></label><label className="text-sm font-bold">Preferred Device<select name="preferredDevicePublicId" className={field}><option value="">不指定</option>{devices.map((device) => <option key={device.publicId} value={device.publicId}>{device.label} · {device.publicId}</option>)}</select></label><label className="text-sm font-bold">Locale<LocaleSelect name="locale" defaultValue="zh-CN" className={field} /></label><label className="text-sm font-bold">Note<input name="note" maxLength={500} className={field} /></label>{error ? <p role="alert" className="border-l-4 border-[var(--signal)] px-4 py-3 text-sm sm:col-span-2">{error}</p> : null}<button disabled={busy || participants.length === 0 || versions.length === 0} className="bg-[var(--signal)] px-5 py-4 font-bold text-white disabled:opacity-50 sm:col-span-2">{busy ? "创建中…" : "创建 Assignment"}</button></form>;
}
