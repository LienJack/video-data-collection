...
export default async function NewAssignmentPage() {
  ...
  const db = database();
  const participants = await db<{ publicId: string; displayAlias: string }[]>`
  ...
  `;
  const versions = await db<{ taskPublicId: string; taskTitle: string; version: number }[]>`
  ...
  `;
  const devices = await db<{ publicId: string; label: string }[]>`
