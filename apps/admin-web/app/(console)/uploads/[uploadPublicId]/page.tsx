...
export default async function AdminUploadPage({ params }: { params: Promise<{ uploadPublicId: string }> }) {
  ...
  return (
    <main className="app-page">
      <Link href="/uploads" className="text-sm font-bold text-[var(--teal)]">← Uploads</Link>
      ...
      <section className="mt-10">
        ...
        {upload.metadata ? <div className="mt-4 overflow-x-auto border border-[var(--line)]"><Table className="w-full min-w-[680px] text-sm"><TableBody>{metadataRows.map(([label, item]) => <TableRow key={label} className="border-t border-[var(--line)] first:border-t-0"><TableHead className="w-52 bg-white/35 p-4 text-left text-[var(--muted)]">{label}</TableHead><TableCell className="p-4">{value(item)}</TableCell></TableRow>)}</TableBody></Table></div> : <p className="mt-4 text-sm text-[var(--muted)]">Metadata 尚不可用；Transfer 状态保持独立。</p>}
      ...
      <section className="mt-10 grid gap-6 xl:grid-cols-2">
        ...
        <Card as="article"><h2 className="display text-2xl font-semibold">相关 ReviewCase</h2><div className="mt-4 space-y-3">{upload.relatedReviews.map((review) => <Link key={review.publicId} href={`/review/${review.publicId}`} className="block border border-[var(--line)] bg-white/35 p-4 font-bold">{review.publicId} · {review.caseType} · {review.status}{review.isFixture ? " · Demo Fixture" : ""}</Link>)}{upload.relatedReviews.length === 0 ? <p className="text-sm text-[var(--muted)]">没有相关 ReviewCase。</p> : null}</div></Card>
      </section>
    </main>
  );
}
