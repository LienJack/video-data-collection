...
export default async function NewTaskPage() {
  ...
  const studies = await db<{ publicId: string; name: string }[]>`
    ...
    order by study.name
  `;
  return <main className="content-page"><Link href="/tasks" className="secondary-action">← Tasks</Link><p className="page-kicker mt-10">Mutable workspace</p><h1 className="page-title">创建 Task Draft</h1><TaskEditor mode="create" studies={studies} initialInstructions={JSON.stringify(defaultTaskInstructions, null, 2)} /></main>;
}
