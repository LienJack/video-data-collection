    ...
    <div className="min-h-[100dvh] lg:grid lg:grid-cols-[264px_1fr]">
      <header className="glass-nav sticky top-0 z-50 mx-2 mt-2 flex items-center justify-between rounded-2xl px-4 py-2.5 text-white lg:hidden">
        ...
        <div className="flex items-center gap-1">
          ...
          <details className="group relative">
            <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-full px-3 text-xs font-semibold text-white/75 hover:bg-white/10"><List className="size-5" weight="duotone" />{t("nav.allFeatures")}</summary>
      ...
      <aside className="glass-nav z-30 mx-3 mt-3 hidden rounded-[26px] px-4 py-4 text-white lg:sticky lg:top-3 lg:ml-3 lg:mr-0 lg:block lg:h-[calc(100dvh-1.5rem)] lg:px-4 lg:py-6">
        <div className="flex items-center justify-between px-2 lg:block">
          ...
          <p className="hidden text-xs font-medium tracking-[0.04em] text-white/45 lg:mt-2 lg:block">{t("nav.videoOperations")}</p>
      ...
      <div className="min-w-0">
        <div className="hidden min-h-16 items-center justify-end gap-3 px-6 lg:flex xl:px-10"><LanguageSwitcher /><SystemGuideLink /></div>
        {children}
      </div>
      <MobileNavigation />
    </div>
