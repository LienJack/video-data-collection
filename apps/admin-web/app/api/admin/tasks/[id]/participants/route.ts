...
export async function GET(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    return apiSuccess(await getTaskOperations(viewer, idSchema.parse(id)), requestId);
  });
}
  ...
  return routeHandler(request, async (requestId) => {
    ...
    return apiSuccess(
      await addTaskParticipants(
        viewer,
        idSchema.parse(id),
        addTaskParticipantsSchema.parse(await request.json()),
