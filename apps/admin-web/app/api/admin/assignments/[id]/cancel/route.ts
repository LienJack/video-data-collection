...
import { assignmentReasonSchema, cancelAssignment } from "@egocapture/core/server/services/tasks";
...
export async function POST(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    const { reason } = assignmentReasonSchema.parse(await request.json());
    return apiSuccess(await cancelAssignment(viewer, idSchema.parse(id), reason, requestId), requestId);
  });
}
