...
import { createDevice, deviceSchema, listDevices } from "@egocapture/core/server/services/participants";
...
export async function GET(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    return apiSuccess(await listDevices(viewer, idSchema.parse(id)), requestId);
  });
}
...
export async function POST(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    const result = await createDevice(
      viewer,
      idSchema.parse(id),
      deviceSchema.parse(await request.json()),
