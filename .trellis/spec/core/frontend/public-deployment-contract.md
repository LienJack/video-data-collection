...
- A server-rendered page that needs multiple independent database results awaits them serially in production code. Do not put queries from the shared `max: 1` pool into `Promise.all`; an abandoned streamed response can otherwise retain the only client and stall later requests on the same Fluid compute instance.
...
| A streamed page leaves a Supavisor query active after its client disconnects | Identify the exact backend by project/application/query, terminate only that abandoned query, serialize same-page query branches, and rerun the filtered route plus full flow. |
