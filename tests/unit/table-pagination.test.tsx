...
describe("TablePagination", () => {
  it("links to adjacent pages while preserving filters", () => {
    render(<TablePagination
      pathname="/records"
      query={{ tab: "videos", search: "needle", page: 2, pageSize: 20 }}
      pagination={{ page: 2, pageSize: 20, totalItems: 51, totalPages: 3 }}
    />);

    expect(screen.getByRole("link", { name: "上一页" })).toHaveAttribute("href", "/records?tab=videos&search=needle&pageSize=20");
    expect(screen.getByRole("link", { name: "下一页" })).toHaveAttribute("href", "/records?tab=videos&search=needle&pageSize=20&page=3");
    ...
    expect(jumpForm?.querySelector('input[type="hidden"][name="pageSize"]')).toHaveValue("20");
    expect(screen.getByRole("combobox", { name: "每页行数" })).toHaveValue("20");
    expect(screen.getByRole("combobox", { name: "每页行数" })).toHaveTextContent("10 行20 行50 行");
  ...
  it("disables the previous control on the first page", () => {
    render(<TablePagination
      ...
      pagination={{ page: 1, pageSize: 20, totalItems: 26, totalPages: 2 }}
    />);
    ...
    expect(screen.getByRole("link", { name: "下一页" })).toHaveAttribute("href", "/tasks?lifecycle=active&pageSize=20&page=2");
  });
  ...
  it("disables the outward control on the final page", () => {
    render(<TablePagination
      ...
      query={{ status: "active" }}
      pagination={{ page: 2, pageSize: 20, totalItems: 26, totalPages: 2 }}
    />);
  ...
  it("keeps an empty result on one stable page without redundant controls", () => {
    ...
    expect(screen.getByRole("combobox", { name: "每页行数" })).toHaveValue("50");
