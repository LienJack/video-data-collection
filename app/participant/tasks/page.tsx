...
export default async function ParticipantTasksPage() {
  ...
  return (
    <main className="content-page max-w-3xl">
      ...
      <Link href="/participant/uploads" className="primary-action mt-6">上传录制文件 →</Link>
      <div className="mt-10 space-y-4">
        {assignments.map((assignment) => (
          <Link key={assignment.publicId} href={`/participant/tasks/${assignment.publicId}`} className="surface-solid block p-6 transition hover:-translate-y-1 hover:shadow-[var(--shadow)]">
