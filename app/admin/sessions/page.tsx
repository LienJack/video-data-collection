...
export default async function AdminSessionsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="app-page">
      ...
      <form className="surface my-7 flex flex-wrap gap-3 p-3"><input name="search" defaultValue={query.search} placeholder="Session / Assignment / Participant / Task" className="min-w-64 flex-1 border border-[var(--line)] bg-[var(--paper)] px-3 py-3" /><select name="status" aria-label="Session Status" defaultValue={query.status ?? ""} className="border border-[var(--line)] bg-[var(--paper)] px-3"><option value="">全部状态</option><option value="open">open</option><option value="closed">closed</option></select><button className="primary-action">筛选</button></form>
      <div className="grid gap-4 xl:grid-cols-2">
        {result.items.map((session) => <article key={session.publicId} className="surface-solid p-6"><div className="flex justify-between gap-4"><p className="text-xs font-bold text-[var(--signal)]">{session.publicId}</p><span className="status-pill">{session.status}</span></div><h2 className="display mt-4 text-2xl font-semibold">{session.taskTitle}</h2><p className="mt-3 text-sm">{session.participantAlias} · {session.participantPublicId}</p><p className="mt-2 text-xs text-[var(--muted)]">{session.deviceLabel} · {session.devicePublicId}</p><p className="mt-2 text-xs text-[var(--muted)]">Marker {session.markerAcknowledgedAt ? "已确认" : "待确认"} · {session.createdAt.toLocaleString("zh-CN")}</p>{session.status === "open" ? <SessionClose sessionPublicId={session.publicId} /> : null}</article>)}
        {result.items.length === 0 ? <p className="empty-state">尚无 Recording Session。</p> : null}
      </div>
