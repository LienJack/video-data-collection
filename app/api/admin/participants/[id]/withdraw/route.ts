...
import { changeParticipantStatus, participantReasonSchema } from "@/src/server/services/participants";
...
export async function POST(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    return apiSuccess(await changeParticipantStatus(viewer, idSchema.parse(id), "withdrawn", reason, requestId), requestId);
  });
}
