...
import { closeSession, sessionReasonSchema } from "@/src/server/services/sessions";
...
export async function POST(request: Request, context: IdContext) {
  return routeHandler(request, async (requestId) => {
    ...
    const { reason } = sessionReasonSchema.parse(await request.json());
    return apiSuccess(await closeSession(viewer, idSchema.parse(id), reason, requestId), requestId);
  });
}
